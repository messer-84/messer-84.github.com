...
if (!window['YT']) {var YT = {loading: 0,loaded: 0};}if (!window['YTConfig']) {var YTConfig = {'host': 'http://www.youtube.com'};}if (!YT.loading) {YT.loading = 1;(function(){var l = [];YT.ready = function(f) {if (YT.loaded) {f();} else {l.push(f);}};window.onYTReady = function() {YT.loaded = 1;for (var i = 0; i < l.length; i++) {try {l[i]();} catch (e) {}}};YT.setConfig = function(c) {for (var k in c) {if (c.hasOwnProperty(k)) {YTConfig[k] = c[k];}}};var a = document.createElement('script');a.type = 'text/javascript';a.id = 'www-widgetapi-script';a.src = 'http://s.ytimg.com/yts/jsbin/www-widgetapi-vflPSGdpW/www-widgetapi.js';a.async = true;var b = document.getElementsByTagName('script')[0];b.parentNode.insertBefore(a, b);})();}
 ...
 * Main JS script.
 */
//temporary info please don't erese
...
function initHeader() {
  ...
  var $wrapper = $('.tm-site');
  var $progress = $header.find('.navbar_progress');
  ...
  function changeProgress() {
    ...
    var pctScrolled = Math.floor($window.scrollTop() / diff * 100);
    pctScrolled = pctScrolled <= 1 ? 1 : pctScrolled; // Min 1%
    requestAnimationFrame(function() {
      $progress.css('width', pctScrolled + '%');
    });
  }

  changeProgress();
  $window.scroll(function() {
    $header.toggleClass('collapsed', $window.scrollTop() >= collapseHeight);
    $wrapper.toggleClass('collapsed', $window.scrollTop() >= collapseHeight);
    changeProgress();
  });
}
...
function photoVideoCarousel(carouselItem) {
  ...
  function initCarouselModule() {
    ...
    carousel.on('init', function (event, slick) {
      ...
      setStartPosition(carouselImagesArray[0]);
    ...
    carousel.on('beforeChange', function (event, slick, currentSlide, nextSlide) {
      ...
      TweenMax.set(carouselImagesArray[nextSlide], {xPercent: 100, opacity: 1, scale: 1});


      if (Math.abs(nextSlide - currentSlide) == 1) {
        directionSlide = (nextSlide - currentSlide > 0) ? "right" : "left";
      }
      else {
        directionSlide = (nextSlide - currentSlide > 0) ? "left" : "right";
      }
      ...
      if (directionSlide === 'right') {
        ...
        TweenMax.fromTo(carouselImagesArray[nextSlide], .7, {xPercent: 100, ease: Power2.easeIn}, {
          xPercent: 0,
          ease: Power2.easeIn
        });

        TweenMax.fromTo(carouselImagesArray[currentSlide], .7, {opacity: 1, scale: 1, ease: Power2.easeIn}, {
          opacity: 0,
          scale: .7,
          ease: Power2.easeIn
        });
        ...
        TweenMax.fromTo(carouselImagesArray[currentSlide], .7, {scale: 1, opacity: 1, ease: Power2.easeIn}, {
          opacity: 0,
          scale: .7,
          ease: Power2.easeIn
        });
...
function initContentOverlay() {
  ...
  function fillData(data) {
    ...
    $overlay.find('.left').css('background-image', 'url("' + image_field + '")');
    
    $content.find('.post_title').html(data.title.rendered);
    
    $content.find('.content').html(data.content.rendered);
    ...
    if ('event_date_start' in data.acf) {
      console.log(data.acf.event_date_start);
...
function initMenuOverlay() {
  ...
  var $footer = $menu.find('.content_footer');
  if (!$menu.length) {
    return;
  }
  var $background = $menu.find('.background');
  var $hamburger = $('.o__header>.hamburger');
  ...
  // Set nav footer top
  function setNavFooterTop() {
    // Only in desktop, because in mobile position = relative.
    if (!isMobile()) {
      $footer.css('top', $content.height() + parseInt($content.css('padding-bottom')) + 10);
    }
  }
...
function initSliderQuotes() {
  ...
  var progress = $('.quotes_progress');
  var $sliderQuotes = $('.slider_quotes_list');
  ...
  $sliderQuotes.on('afterChange', function (event, slick, currentSlide) {
...
function initTwoCards() {
  var $hero = $('.o__two_hero_cards');
  var $cards = $hero.find('.card');
  var padding = 38;
  var margin = 60;

  /**
   * Hover animations on cards. Just trigger some
   * classes that handle the animation.
   */
  $cards.hover(function() {
    if (!isBelowTablet()) {
      var $card = $(this);
      var $title = $card.find('.title');
      $card.find('.card_info').addClass('animate');
      $card.find('.separator').addClass('animate');
      var offset = $title.next().height() + margin + padding;
      $title.css({'bottom': offset + 'px'})
    }
  }, function() {
    if (!isBelowTablet()) {
      var $card = $(this);
      var $title = $card.find('.title');
      $card.find('.card_info').removeClass('animate');
      $card.find('.separator').removeClass('animate');
      $title.css({'bottom': margin + 'px'})
    }
  });
...
function initSidebarFunctions() {
  ...
  function initSidebarFixed() {
    ...
    if ($scrollTopGl > site_h - (sidebarH + footer_h)) {
    ...
    if ($scrollTopGl > sidebarTop - header_h) {
